...
import {OverlayPositionChangeListener} from "./definitions";

export interface CapacitorGoogleMapLatLng {
  lat : number,
  lng : number
}
...
export interface GoogleMapInterface {
  ...
  removeOverlayPositionChangeListener(callbackId: string) : Promise<void>;
  ...
  /*addOverlayPositionChangeListener(
      callback?: MapListenerCallback<OverlayPositionChangeListener>,
...
export class GoogleMap {
  ...
  async resize(_el : HTMLElement) : Promise<void> {
    const elementBounds = await GoogleMap.getElementBounds(_el);
    return CapacitorGoogleMaps.resize({
      id : this.id,
  ...
  async removeOverlayPositionChangeListener(callbackId: string) : Promise<void> {
    return CapacitorGoogleMaps.removeOverlayPositionChangeListener({
      id: this.id,
  ...
  */
  ...
  async addOverlayPositionChangeListener(_latLng : CapacitorGoogleMapLatLng,
      callback? : MapListenerCallback<OverlayPositionChangeListener>
  ) : Promise<PluginListenerHandle>{
    if( !callback )
      throw "callback is null";

      return CapacitorGoogleMaps.addListener(
          `addOverlayPositionChange,${this.id},${_latLng.lat},${_latLng.lng}`,
          this.generateCallback(callback)
      );
  }
