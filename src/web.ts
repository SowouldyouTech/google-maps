...
import type {
  ...
  TrafficLayerArgs,
  RemoveMarkersArgs,
  OnScrollArgs, ToScreenLocationArgs, RemoveOverlay, OverlayCallback,
} from './implementation';
import {ResizeArgs} from "./implementation";
  ...
  implements CapacitorGoogleMapsPlugin
{
  ...
  private overlayCallbacks : Record<string, OverlayCallback[]> = {};
  private overlayCallbackIds  = 0;
  private currMarkerId = 0;

  public async resize(bounds : ResizeArgs) : Promise<void> {
    bounds;
  }
  ...
  public async removeOverlayPositionChangeListener(_args :RemoveOverlay) : Promise<void> {
    if( this.overlayCallbacks[_args.id] )
    {
      const idx = this.overlayCallbacks[_args.id].findIndex(x => x.callbackId === _args.callbackId);
      if( idx > -1 )
        this.overlayCallbacks[_args.id].splice(idx, 1);
    }
  }

  public addListener(eventName: string, listenerFunc: (...args: any[]) => any): Promise<PluginListenerHandle> & PluginListenerHandle {
    if( eventName.startsWith("addOverlayPositionChange"))
    {
      const splitArr = eventName.split(",");
      if( splitArr.length < 4)
        throw splitArr;
      const mapId = splitArr[1];
      const lat = parseFloat(splitArr[2]);
      const lng = parseFloat(splitArr[3]);
      const id = this.overlayCallbackIds.toString();
      ++this.overlayCallbackIds;

      if( !this.overlayCallbacks[mapId] )
        this.overlayCallbacks[mapId] = [];
      
      this.overlayCallbacks[mapId].push({
        mapId,
        lat,
        lng,
      ...
      const _this : CapacitorGoogleMapsWeb = this;
      const removeHandle : PluginListenerHandle = {
        async remove() {
          await _this.removeOverlayPositionChangeListener({ id, callbackId : id });
        }
      };
      const promise = new Promise<PluginListenerHandle>((resolve) => {
        resolve(removeHandle);
      });
  ...
  toScreenLocation(_args: ToScreenLocationArgs): any {
    const map = this.maps[_args.id];

    return map.canvasProjectionOverlay.getProjection().fromLatLngToContainerPixel({ lat : _args.lat, lng : _args.lng });
  }
  ...
  async create(_args: CreateMapArgs): Promise<void> {
    ...
    const ctx = class CanvasProjectionOverlay extends google.maps.OverlayView{
      constructor() {
        super();
      }
    ...
    this.maps[_args.id] = {
      ...
      markers: {},
      canvasProjectionOverlay : new ctx()
    };
    this.maps[_args.id].canvasProjectionOverlay.setMap(this.maps[_args.id].map);
  ...
  async setMapListeners(mapId: string): Promise<void> {
    ...
    const overlayFunc = () => {
      if( this.overlayCallbacks[mapId] )
      {
        for(const item of this.overlayCallbacks[mapId]) {
          const position = this.toScreenLocation({ id : mapId, lng : item.lng, lat : item.lat });
          item.callback(Object.assign({ mapId : mapId }, position));
        }
      }
    }
    ...
    map.addListener('bounds_changed', async () => {
      ...
      this.notifyListeners('onCameraMove', {
        mapId: mapId,
        bearing: map.getHeading(),
        bounds: bounds,
        latitude: map.getCenter()?.lat(),
