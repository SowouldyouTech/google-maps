...
class CapacitorGoogleMap(
    ...
    private var lastDrawTime: LocalTime;

    init {
        val bridge = delegate.bridge
        config.googleMapOptions?.backgroundColor("e8e5ee".toInt(16));
        mapView = MapView(bridge.context, config.googleMapOptions)
        initMap()
        setListeners()
        lastDrawTime = LocalTime.now();
    }
    ...
    fun resize(x : Int, y : Int, w : Int, h : Int) {
        CoroutineScope(Dispatchers.Main).launch {
            var zoom = this@CapacitorGoogleMap.googleMap?.cameraPosition?.zoom ?: 0;
            var bounds = getLatLngBounds();
            ...
            val layoutParams =
                    FrameLayout.LayoutParams(
                            getScaledPixels(delegate.bridge, w),
                            getScaledPixels(delegate.bridge, h))

            layoutParams.leftMargin =getScaledPixels(delegate.bridge, x)
            layoutParams.topMargin = getScaledPixels(delegate.bridge, y)
            mapView.layoutParams = layoutParams;
    ...
    fun setListeners() {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            this@CapacitorGoogleMap.googleMap?.setOnCameraMoveListener(
    ...
    override fun onCameraMove() {
        ...
        //val duration = Duration.between(lastDrawTime, now);
