...
public class Map {
    ...
    func render() {
        DispatchQueue.main.async {
            ...
            self.mapViewController.GMapView.setMinZoom(self.config.minZoom, maxZoom: self.config.maxZoom)
            self.mapViewController.GMapView.backgroundColor = self.hexStringToUIColor(hex: "#e8e5ee")
    ...
    func resize(x : Int, y : Int, w : Int, h : Int) {
        DispatchQueue.main.async {
            
                let currentCamera = self.mapViewController.GMapView.camera
            
            var zoom = currentCamera.zoom
            var bounds = self.getMapLatLngBounds()
            var center = currentCamera.target;
            
            self.mapViewController.view.frame = CGRect(x : x, y: y, width : w, height: h)
            self.mapViewController.view.layoutIfNeeded()
            
            let newCamera = GMSCameraPosition(latitude: center.latitude,
