...
public class CapacitorGoogleMapsPlugin: CAPPlugin, GMSMapViewDelegate {
    ...
    @objc func resize(_ call: CAPPluginCall) {
        do {
            ...
            guard let bounds = call.getObject("bounds") else {
                throw GoogleMapErrors.invalidArguments("bounds object is missing")
            }
            
            guard let x = bounds["x"] as? Int else {
                throw GoogleMapErrors.invalidArguments("bounds object is missing")
            }
            
            guard let y = bounds["y"] as? Int else {
                throw GoogleMapErrors.invalidArguments("bounds object is missing")
            }
            
            guard let w = bounds["width"] as? Int else {
                throw GoogleMapErrors.invalidArguments("bounds object is missing")
            }
            
            guard let h = bounds["height"] as? Int else {
                throw GoogleMapErrors.invalidArguments("bounds object is missing")
            }

            if self.maps[id] != nil {
                self.maps[id]?.resize(x : x, y : y, w : w, h : h)
            }
    ...
    // onCameraMove
    public func mapView(_ mapView: GMSMapView, didChanged cameraPosition: GMSCameraPosition) {
        let mapId = self.findMapIdByMapView(mapView)
        let map = self.maps[mapId]
        let bounds = map?.getMapLatLngBounds()
        ...
        self.notifyListeners("onCameraMove", data: data)
    }
